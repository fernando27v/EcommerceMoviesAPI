  ...
  try {
    ...
    movies.forEach(async (m) => {
      ...
      const actorsDetail = await Promise.all([
        axios.get(
          `https://api.themoviedb.org/3/person/${actors.data.cast[0].id}?api_key=${API_KEY}&language=en-US`
        ),
        axios.get(
          `https://api.themoviedb.org/3/person/${actors.data.cast[1].id}?api_key=${API_KEY}&language=en-US`
        ),
      ]);
      ...
      apiInfo.forEach(async (el) => {
        try {
          const a = await Actor.findOrCreate({
            where: {
              id: el.id,
              name: el.name,
          ...
          await actor_movie.findOrCreate({
            where: { ActorId: el.id, MovieId: p.id },
          });
        } catch (err) {
